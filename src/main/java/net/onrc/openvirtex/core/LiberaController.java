...
public class LiberaController implements Runnable {

    Logger log = LogManager.getLogger(LiberaController.class.getName());

    private static final int SEND_BUFFER_SIZE = 1024 * 1024;
    private static LiberaController instance = null;
    ...
    public LiberaController(CmdLineSettings settings) {
        ...
        LiberaController.instance = this;
        LiberaController.tenantIdCounter = new BitSetIndex(
                IndexType.TENANT_ID);
    }

    @Override
    public void run() {
        Runtime.getRuntime().addShutdownHook(new LiberaShutdownHook(this));
        initVendorMessages();
            ...
            cf.addListener(new ChannelFutureListener() {
                ...
                public void operationComplete(final ChannelFuture e)
                    ...
                    if (e.isSuccess()) {
                        ...
                        sw.setChannel(chan);
                        LiberaController.this.cg.add(chan);
                    } else {
                        LiberaController.this.log
    ...
    private void setServerBootStrapParams(final ServerBootstrap bootstrap) {
        ...
        bootstrap.setOption("child.sendBufferSize",
                LiberaController.SEND_BUFFER_SIZE);
    ...
    public static LiberaController getInstance() {
        if (LiberaController.instance == null) {
            throw new RuntimeException(
                    "The LiberaController has not been initialized; quitting.");
        }
        return LiberaController.instance;
    }
